...
renwu = ["达到2级（加20攻击）","达到3级（加20防御）","达到5级（赠送50金币）","达到10级（土地自由（增加50空地））","达到25级（加666生命）","达到100级（金币自由（加10000金币））"]
renwuxuanze = 0
tiaozhanxuanze = 0
queding = 0
kuaisu = 0
...
# 函数定义
def fast():
    global kuaisu,juesefangyu,juesegongji,chongwugongji,chongwufangyu,chongwushengming,chongwudengji,juesedengji,jueseshengming,jinbi
    kuaisu=int(input("是否进入快速模式？（1、是 2、否）"))
    if kuaisu==1:
        kuaisu=int(input("升级几次"))
        for l in range(kuaisu):
...
def x14():
    global juesedengji,gongdianxuanze,wangguorenshu,wangguoshuishou,wangguotudi,juminmanyidu,guoku
    if juesedengji>=20:
        print("欢迎来到宫殿")
        while True:
            wangguorenshu=5
            juminmanyidu=1000/wangguoshuishou
            gongdianxuanze = int(input("1、说明 2、查看王国数据 3、操作 4、退出"))
            if gongdianxuanze==1:
                print("你已经级别很高了，于是你决定建立一个王国。\n你可以在这个王国里建造各种东西，也可以干很多事情\n你也可以进行战争，增加土地\n当然，也可能遭遇战争，这就需要我们前期生产良好\n开始吧！")
            elif gongdianxuanze==2:
                print("王国人数：{}\n王国税收：{}\n居民满意度：{}\n王国国库{}\n王国土地：{}".format(wangguorenshu,wangguoshuishou,juminmanyidu,guoku,wangguotudi))
            elif gongdianxuanze==3:
                print("暂未开放")
                #gongdianxuanze = int(input("1、增加税收 2、减少税收"))
                #if gongdianxuanze==1:
                #    print("当前税收：".format(wangguoshuishou))
                #    gongdianxuanze = int(input("你要上升多少?"))
    else:
        print("20级以上才能解锁本模块")
...
while True:
    try:
        ...
        tiaozhan = 0
        xuanze = int(input(
            "1、角色查看 2、宠物老虎查看 3、总值查看 4、战斗 5、规则 6、退出 7、胜率查看 8、田地 9、练习场 10、宝箱 11、技能 12、任务 13、挑战模式 14、王国"))
