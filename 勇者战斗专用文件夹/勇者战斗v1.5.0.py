juesedengji = 1000#000
jueseshengming = 30000#000
juesefangyu = 2500#00
juesegongji = 35000#000
chongwudengji = 1
chongwushengming = 5
chongwufangyu = 5
chongwugongji = 10
...
jinengshangdian=["连发（攻击加倍）","快速（使敌人一回合无法攻击，并进行普通攻击）","恢复（恢复当前生命的1.25倍）"]
...
def x4(juesedengji, jueseshengming, juesefangyu, juesegongji, chongwudengji, chongwushengming, chongwufangyu,
    ...
    while wofangshengming >= 0 and direnshengming >= 0:
        ...
        elif jineng[zuozhanxuanze-1] == "快速（使敌人一回合无法攻击，并进行普通攻击）":
            print("快速--造成攻击：")
            print(zonggongji - direnfangyu)
            direnshengming = direnshengming - (zonggongji - direnfangyu)
            continue
    ...
    if renwuxuanze == 0:
        ...
        if renwu[renwuxuanze-1] == "达到2级（加20攻击）":
            ...
            if juesedengji>=5:
                ...
                print("还未达到要求")
        elif renwu[renwuxuanze - 1] == "达到10级（土地自由（增加50田地））":
            if juesedengji>=10:
                for t in range(50):
                    tiandi.append("田地")
                print("兑换成功！")
...
while True:
    try:
        ...
        if xuanze == 1:
            ...
            queding=int(input("确定退出吗？1、确定 2、取消（退出后不保存记录）"))
            if queding == 1:
                break
            else:
                pass
