...
l3 = pygame.image.load('../素材/里程碑3.png')
lt1 = 0
lt2 = 0
lt3 = 0
dan_list = []
...
pygame.mixer.music.load('../素材/自由.wav')
...
while True:
    ...
    word5=font1.render('积分：{}'.format(fen),True,(0,0,0))
    window.blit(word5,(0,660))
    if ming>=50 and lt1<=500:
        lt1+=1
        window.blit(l1,(150,100))
    if gametime>=600 and lt2<=500:
        lt2+=1
        window.blit(l2,(150,100))
    if fen>=10000 and lt3<=500:
        lt3+=1
        window.blit(l3,(150,100))
    ...
    try:
        for i in range(len(dan_list)):
            if dan_list[i]=='one':
                ...
                if dan_y[i]<=0:
                    del dan_list[i], dan_x[i], dan_y[i]
            elif dan_list[i]=='right':
                dan_x[i] += 5
                window.blit(dan, (dan_x[i], dan_y[i]))
                if (dan_x[i] <= stone_rect.x + 134 and dan_x[i] >= stone_rect.x) and (
                        dan_y[i] <= stone_rect.y + 99 and dan_y[i] >= stone_rect.y):
                    have = 0
                    del dan_list[i], dan_x[i], dan_y[i]
                    fen += 100
                if dan_x[i] >= 500:
    ...
    for event in pygame.event.get():
        ...
        if keys[pygame.K_RIGHT]:
            dan_list.append("right")
            dan_x.append(px + 20)
            dan_y.append(510)
            wait = gametime
        if keys[pygame.K_LEFT]:
            dan_list.append("left")
            dan_x.append(px + 20)
            dan_y.append(510)
            wait = gametime
    keys = pygame.key.get_pressed()
